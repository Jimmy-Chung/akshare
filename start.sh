...
start_heatmap_watcher() {
  ...
  screen -dmS "$screen_session" /bin/zsh -lc \
    "unset http_proxy https_proxy all_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY; cd '$ROOT' && exec '$WATCHER_PYTHON_BIN' '$ROOT/tools/market_heatmap_timeline.py' --market '$market' --session '$session' --port '$port' watch --interval-seconds 1800 >> '$LOG_DIR/heatmap-${lower_market}.log' 2>&1"
}
...
case "${1:-start}" in
  start)
    ...
    echo "采集:  CN/HK/US 热力图 watcher 已随应用启动"
    echo "日志:  $LOG_DIR/"
    ;;
  ...
  status)
    ...
    for market in cn hk us; do
      ...
      if screen -ls | grep -q "${HEATMAP_WATCHER_PREFIX}-${market}"; then
